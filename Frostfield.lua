--[[
	Frostfield
	Rayfield-compatible UI library — ice / frost chrome.
	Usage:
		local Frostfield = loadstring(game:HttpGet("YOUR_RAW_URL"))()
		-- or paste this file as a ModuleScript and require it
]]
...
local Theme = {
	...
	TextMuted = Color3.fromRGB(183, 212, 240),
	Background = Color3.fromRGB(11, 26, 54),
	Topbar = Color3.fromRGB(18, 40, 80),
	TabBar = Color3.fromRGB(22, 52, 108),
	Element = Color3.fromRGB(40, 80, 160),
	ElementBorder = Color3.fromRGB(90, 160, 220),
	Highlight = Color3.fromRGB(100, 180, 255),
	Ice = Color3.fromRGB(126, 200, 255),
}
...
local function switchTab(id)
	...
	for key, tab in pairs(State.tabs) do
		local on = key == id
		tab.page.Visible = on
		tab.button.BackgroundTransparency = on and 0.55 or 1
...
local function buildChrome(opts)
	...
	window.Parent = gui
	round(window, 12)
	stroke(window, Theme.Ice, 1, 0.35)
	State.window = window
	...
	frost.ZIndex = 0
	if opts.BackgroundImage and opts.BackgroundImage ~= 0 and opts.BackgroundImage ~= "" then
		local img = opts.BackgroundImage
		frost.Image = type(img) == "number" and ("rbxassetid://" .. img) or tostring(img)
	else
		frost.Image = ""
		frost.BackgroundColor3 = Theme.Background
		frost.BackgroundTransparency = 1
	end
	frost.Parent = window
...
function TabApi:CreateToggle(opts)
	...
	function api:Set(v)
		...
		paint(value)
		if opts.Callback and not busy then
			busy = true
