...
local Theme = {
	...
	TextMuted = Color3.fromRGB(210, 225, 240),
	Background = Color3.fromRGB(48, 78, 118),
	Topbar = Color3.fromRGB(68, 96, 132),
	TabBar = Color3.fromRGB(58, 86, 122),
	Element = Color3.fromRGB(145, 180, 220),
	ElementBorder = Color3.fromRGB(190, 220, 245),
	Highlight = Color3.fromRGB(130, 175, 220),
	Ice = Color3.fromRGB(200, 230, 255),
	Neon = Color3.fromRGB(170, 215, 255),
}
...
local function loadRemoteImage(imageLabel, url)
	task.spawn(function()
		...
		pcall(function()
			local asset = (getcustomasset and getcustomasset(path)) or (getsynasset and getsynasset(path))
...
local function drawGear(parent)
	...
	disc.Size = UDim2.fromOffset(11, 11)
	...
	hole.BackgroundColor3 = Color3.fromRGB(70, 105, 145)
...
local function drawDiscord(parent)
	local img = Instance.new("ImageLabel")
	img.Name = "DiscordIcon"
	img.BackgroundTransparency = 1
	img.Size = UDim2.fromOffset(18, 18)
	img.AnchorPoint = Vector2.new(0.5, 0)
	img.Position = UDim2.new(0.5, 0, 0, 7)
	img.Image = "rbxassetid://6022628378"
	img.ImageColor3 = Color3.fromRGB(185, 230, 255)
	img.ScaleType = Enum.ScaleType.Fit
	img.Parent = parent
	return img
end
...
function Frostfield:Notify(opts)
	...
	round(card, 16)
	stroke(card, Color3.fromRGB(190, 220, 250), 1.2, 0.25)
	...
	title.Text = toEnglish(tostring(opts.Title or "Notice"))
	title.Parent = wrap
	...
	body.Text = toEnglish(tostring(opts.Content or ""))
	body.Parent = wrap
...
local function setMinimized(v)
	...
	if v then
		...
		State.window.Size = UDim2.fromOffset(State.window.AbsoluteSize.X, 44)
	else
		local h = State.expanded and 640 or 510
		State.window.Size = UDim2.fromOffset(State.expanded and 640 or 580, h)
...
local function setExpanded(v)
	...
	if v then
		State.window.Size = UDim2.fromOffset(640, 640)
	else
		State.window.Size = UDim2.fromOffset(580, 510)
	end
end
...
local function makeTopButton(parent, text, order, callback)
	...
	track(b.MouseEnter:Connect(function()
		...
		b.BackgroundColor3 = Color3.fromRGB(180, 210, 240)
	end))
...
local function makeElement(parent)
	...
	f.Parent = parent
	round(f, "pill")
	local st = stroke(f, Color3.fromRGB(210, 230, 250), 1, 0.42)
	track(f.MouseEnter:Connect(function()
		tween(f, 0.12, { BackgroundTransparency = 0.2 })
		st.Transparency = 0.22
	end))
	track(f.MouseLeave:Connect(function()
		tween(f, 0.12, { BackgroundTransparency = 0.34 })
...
local function switchTab(id)
	...
	for key, tab in pairs(State.tabs) do
		local on = key == id
		tab.page.Visible = on
		tab.button.BackgroundTransparency = on and 0.42 or 1
		tab.button.BackgroundColor3 = Color3.fromRGB(110, 145, 185)
		tab.button.TextColor3 = on and Theme.TextColor or Theme.TextMuted
		if tab.stroke then
			tab.stroke.Transparency = on and 0.5 or 1
		end
		if tab.icon then
...
local function buildChrome(opts)
	...
	chip.Parent = gui
	round(chip, 12)
	stroke(chip, Color3.fromRGB(190, 220, 250), 1, 0.45)
	State.chip = chip
	...
	window.Size = UDim2.fromOffset(580, 510)
	window.BackgroundColor3 = Color3.fromRGB(40, 68, 105)
	window.BackgroundTransparency = 0.72
	window.BorderSizePixel = 0
	window.ClipsDescendants = true
	window.Parent = gui
	round(window, 18)
	stroke(window, Color3.fromRGB(190, 220, 250), 1.4, 0.35)
	State.window = window
	...
	frost.Position = UDim2.fromScale(0, 0)
	frost.ImageTransparency = 0.06
	frost.ImageColor3 = Color3.fromRGB(210, 228, 255)
	...
	glass.BackgroundColor3 = Color3.fromRGB(70, 105, 155)
	glass.BackgroundTransparency = 0.84
	...
	top.BackgroundColor3 = Color3.fromRGB(66, 94, 128)
	...
	flake.Position = UDim2.fromOffset(10, 0)
	flake.Size = UDim2.fromOffset(26, 44)
	flake.Font = Enum.Font.Gotham
	flake.TextSize = 18
	flake.Text = "❄"
	flake.TextColor3 = Theme.Ice
	flake.ZIndex = 3
	flake.Parent = top
	...
	title.TextXAlignment = Enum.TextXAlignment.Left
	title.TextColor3 = Theme.TextColor
	title.Text = opts.Name or "Frost Hub Universal Desync"
	title.ZIndex = 3
	title.Parent = top
	...
	sidebar.BackgroundColor3 = Color3.fromRGB(100, 135, 175)
	...
	content.Position = UDim2.fromOffset(96, 0)
	...
	footer.BackgroundColor3 = Color3.fromRGB(66, 94, 128)
...
function TabApi:CreateToggle(opts)
	...
	sw.Size = UDim2.fromOffset(40, 22)
	sw.BackgroundColor3 = value and Color3.fromRGB(90, 140, 190) or Color3.fromRGB(50, 68, 95)
	...
	knob.Position = value and UDim2.new(1, -19, 0.5, -8) or UDim2.fromOffset(3, 3)
	...
	local function paint(v)
		sw.BackgroundColor3 = v and Color3.fromRGB(90, 140, 190) or Color3.fromRGB(50, 68, 95)
		knob.Position = v and UDim2.new(1, -19, 0.5, -8) or UDim2.fromOffset(3, 3)
	end
...
function TabApi:CreateParagraph(opts)
	...
	title.TextXAlignment = Enum.TextXAlignment.Left
	title.TextColor3 = Theme.TextColor
	title.Text = toEnglish(opts.Title or "")
	title.Parent = wrap
	...
	body.TextXAlignment = Enum.TextXAlignment.Left
	body.TextColor3 = Theme.TextMuted
	body.Text = toEnglish(opts.Content or "")
	body.Parent = wrap
...
function TabApi:CreateDropdown(opts)
	...
	valueLbl.Text = toEnglish(tostring(current))
	...
	holder.Parent = wrap
	round(holder, 16)
	stroke(holder, Color3.fromRGB(210, 230, 250), 1, 0.4)
	pad(holder, 6)
	...
	local function selectOpt(opt)
		current = opt
		valueLbl.Text = toEnglish(tostring(opt))
		if flag then
...
function TabApi:CreateSection(opts)
	local title = type(opts) == "string" and opts or (opts and (opts.Name or opts.Title)) or "Section"
	self._root = self._root or self._list
	...
	round(box, 16)
	stroke(box, Color3.fromRGB(210, 230, 250), 1, 0.38)
	pad(box, 8)
	local lay = Instance.new("UIListLayout")
	lay.SortOrder = Enum.SortOrder.LayoutOrder
	lay.Padding = UDim.new(0, 8)
	...
	head.TextColor3 = Theme.Ice
	head.Text = toEnglish(title)
	head.Parent = box
...
function WindowApi:CreateTab(name, image)
	...
	round(btn, "pill")
	local tabStroke = stroke(btn, Color3.fromRGB(200, 225, 250), 1, 1)

	local icon
	local n = string.lower(tostring(name or ""))
	if n:find("setting") or n:find("config") then
		icon = drawGear(btn)
	elseif n:find("discord") then
		icon = drawDiscord(btn)
	else
	...
	label.Text = toEnglish(tostring(name or "Tab"))
	label.Parent = btn
	btn.TextColor3 = Theme.TextMuted

	local page, list = createPage()
	State.tabs[id] = { button = btn, page = page, icon = icon, name = name, stroke = tabStroke }
