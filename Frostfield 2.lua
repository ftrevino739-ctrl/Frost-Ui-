...
local Theme = {
	...
	Background = Color3.fromRGB(28, 78, 150),
	Topbar = Color3.fromRGB(36, 98, 180),
	TabBar = Color3.fromRGB(32, 92, 175),
	Element = Color3.fromRGB(48, 130, 230),
	ElementBorder = Color3.fromRGB(150, 220, 255),
	Highlight = Color3.fromRGB(130, 220, 255),
	Ice = Color3.fromRGB(170, 235, 255),
	Neon = Color3.fromRGB(90, 230, 255),
}
...
local function makeElement(parent)
	...
	f.Parent = parent
	round(f, 8)
	local st = stroke(f, Theme.Ice, 1.4, 0.25)
	track(f.MouseEnter:Connect(function()
		tween(f, 0.12, { BackgroundTransparency = 0.02 })
		st.Transparency = 0.05
		st.Color = Theme.Neon
	end))
	track(f.MouseLeave:Connect(function()
		tween(f, 0.12, { BackgroundTransparency = 0.08 })
...
local function makeNeonButton(parent)
	...
	f.BackgroundColor3 = Color3.fromRGB(40, 150, 255)
...
local function switchTab(id)
	...
	for key, tab in pairs(State.tabs) do
		local on = key == id
		tab.page.Visible = on
		tab.button.BackgroundTransparency = on and 0.15 or 1
...
local function buildChrome(opts)
	...
	chip.Parent = gui
	round(chip, 18)
	stroke(chip, Theme.Neon, 1.5, 0.1)
	State.chip = chip
	...
	window.Parent = gui
	round(window, 12)
	stroke(window, Theme.Neon, 1.6, 0.12)
	State.window = window
